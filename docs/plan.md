...
- [x] Move the repository toolchain to a Node.js `22.13.0` and npm `10.9.2` baseline that matches the modern VS Code extension host.
...
- Regenerate and commit `package-lock.json` under Node.js `22.13.0` with npm `10.9.2`; the current local VM still exposes Node.js `10.24.0` and npm `5.8.0`, which cannot reliably run the supported npm dependency resolver.
